...
class Simulation:
            ...
            self.simulation_dictionary = {self.default_state : {
                                ...
                                'transition_parameters' : {
                                        "TRANSITION_PROBABILITY" : 
                                            (0.009, YEAR, simutils.ncp),
                                        "TRANSITION_BACK_PROBABILITY" : 
                                            (0.7, YEAR, simutils.ncp),        
                                        },
                                },
                        }
    ...
    def _normalise(self, parameters):
        ...
        for parameter in parameters.items():
            ...
            if time_period_from != self.time_period:
                try:
                    normaliser_function = parameter[1][2]
                except IndexError:
                    parameter_dict[parameter[0]] = parameter[1][0]
                else:
                    parameter_dict[parameter[0]] = \
                        normaliser_function(parameter[1][0],
                                            time_period_from,                                             
                                            self.time_period)
            else:     
                parameter_dict[parameter[0]] = parameter[1][0]
    ...
    def proportion(values):
        return ("proportion", simutils.proportion(values))
        
    @staticmethod
    def mean(values):
        return ("mean", 
                simutils.mean(values) )
    
    @staticmethod
    def median(values):
        return ("median", simutils.median(values))
